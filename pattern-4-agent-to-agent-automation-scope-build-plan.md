...
- Releasing the updated `pattern4ce.askGenie` chart-result contract is pending explicit user approval. Until released, the published app can render preview charts but live Genie calls from Domo will not include `columns` + `dataRows`.
...
3. DONE locally — Genie Workspace chart rendering: `askGenie` source returns columns+rows and the app renders Domo-side KPI/line/bar/scatter/table visuals. Await explicit user approval before creating/releasing the live `pattern4ce` version.
4. Sprint 8 (approved Lakebase reuse): create `p4_scenario_runs` + `p4_prediction_feedback` in `cobra-v1`, add Lakebase functions to `pattern4ce`, build the Lakebase Ops page.
5. Sprint 7 (gated by user confirm — serving cost): train renewal-risk/churn model on `gold_customer_renewal_risk`, register via MLflow/UC, deploy Model Serving endpoint; add `pattern4ce.runModelInference` (direct serving call) + ML Predictions page.
6. Keep UC row-filter/PDP alignment and Agent Catalyst/Workflow approval wrapper in scope as supporting governance and automation work.
