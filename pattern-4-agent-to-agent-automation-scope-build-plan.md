...
- 2026-06-14: **Domo PDP ⇄ Databricks RLS scope re-opened — SHAPED (no build).** Per user, re-exploring the original-plan PDP piece: Databricks UC row-level policy = source of truth, replicated into Domo PDP, with a dataset-level demo UI, over the gold views only. Discovery: **no UC row filters exist** on any `gold_*` object (verified); `dim_user_entitlement`/`gold_portal_user_scope` already encode tiers ALL/REGION/TENANT/OWNER × region/tenant_id/account_owner_id; gold objects are **VIEWs** (row-filter-on-view is an open question); scoping columns vary per view; 5/6 gold are federated into Domo (forecast not); embedded app uses a service token (no OBO). Authored `pattern-4-domo-pdp-rls-shaping.md` (frame + CURRENT + R0–R8 + shapes A live-PDP / B PDP-spec+simulated-parity / C true-OBO + macro fit check + spikes X1 Domo-PDP-API-on-federated, X2 UC-row-filter-on-views, X3 OBO). Recommended next: run read-only spikes X1+X2, then pick A vs B. No Databricks/Domo writes, no app/CE changes.
- 2026-06-13: **AI Readiness UC→Domo synonyms sync built (app live-wired; CE staged, NOT released).** Diagnosed why Sync updated Domo Context but not Synonyms: the tab read a STATIC `public/ai-readiness-detail.json` (so the synced `synonyms` were whatever was in the file — empty for the tested column) and never read live UC; and `getUcReadinessState` only derived synonyms from a `domo_ai_synonym` tag that didn't exist. Decisions (user): UC is source of truth (comment + tags); **UC comment → Domo Context, UC tags → Domo Synonyms as a 1:1, Domo-friendly comma-separated copy**; read UC live and replicate straight into Domo. Implemented after a full due-diligence pass (live-tested both Databricks calls; mapped every consumer; confirmed no manifest/alias change → no card re-instantiation): **CE (`functions.js`, build-only):** `getUcReadinessState` now returns per column both `synonyms` (TRUE business synonyms parsed from the `domo_ai_synonyms` tag — for the inspector edit round-trip, so governance facets never leak into the editable field) and a new `domoSynonyms` = true synonyms (bare) + every substantive tag as humanized `key: value`, excluding housekeeping `domo_ai_ready`/`domo_ai_updated_by`. No signature change. **App (UI-only, src+dist):** new `refreshUcReadinessLive()` overlays each dataset's columns from live `getUcReadinessState(item.object)` on load (falls back to the static file if unavailable); `selectedColumnPayload` now sends `domoSynonyms` as the Domo synonyms (falls back to true synonyms pre-release); gauge + column "N synonyms" count reflect the Domo-bound list; inspector still edits true synonyms only. `wipeDomoAiReadiness` already clears both description+synonyms, so sync↔wipe stay mirror-images (no change needed). Validated: `node --check` (functions.js, src+dist app.js) clean, lints clean, helper logic unit-tested (governance tags → `semantic type: measure` etc.; housekeeping excluded; true synonyms kept separate), `src`↔`dist` in sync. Staged `scripts/release_pattern4ce_1020.py` (built from FULL `functions.js`, **not run**). **GATES (user):** (1) **"release"** approval → run `release_pattern4ce_1020.py` (no manifest/alias change → no card re-instantiation); (2) `domo publish` latest `dist`; (3) live-verify Domo persists the multi-entry synonyms array (Step 4). Optional polish: add real business-synonym `domo_ai_synonyms` tags to headline columns so Synonyms shows alternate names alongside the governance facets.
