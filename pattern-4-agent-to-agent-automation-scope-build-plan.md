...
- Databricks external lineage creation is blocked until a metastore admin grants the current principal `CREATE EXTERNAL METADATA` on `metastore_aws_us_east_1` or creates/transfers the external metadata object `domo_pattern4_revenue_command_center`; after that, add downstream relationships from each `gold_*` table to the external app object.
...
7. Ask a Databricks metastore admin to grant `CREATE EXTERNAL METADATA` on metastore `metastore_aws_us_east_1` to the current Pattern 4 principal, then rerun external lineage creation for all six `gold_*` tables.
