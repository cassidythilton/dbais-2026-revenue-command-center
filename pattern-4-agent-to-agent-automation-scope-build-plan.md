...
3. **ML Predictions wiring — app config DONE, live test pending user.** `runModelInference` is mapped in `manifest.json`/`dist` and released in **v1.0.12**; the app's live path is enabled and reads churn prob from `predictions[0]` (index-1), matching the serving contract. Endpoint `pattern4-renewal-risk` READY on `pattern4_renewal_risk_v5`. Remaining (user): publish `dist`, re-instantiate the App Studio card, hard-refresh, click-test live inference.
