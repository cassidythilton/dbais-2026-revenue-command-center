...
- Domo AI Readiness write automation is built locally but not released live: `pattern4ce` now has the required functions, but the `domo_account` Code Engine Account parameter must be configured to the Domo Access Token account before release.
