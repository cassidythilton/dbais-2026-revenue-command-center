...
2. DONE — **AI Readiness tab visual/UX redesign (Shape C).** Shaped first (`pattern-4-ai-readiness-redesign-shaping.md`), user picked Shape C, built front-end-only against the frozen CE contract, validated locally (incl. headless render of the control plane + UC inspector drawer). Awaiting user publish.
3. **ML Predictions — model fixed + tab reworked; live test pending user.** Endpoint `pattern4-renewal-risk` now serves UC **v6 (regressor)** returning a smooth churn probability (~0.10–0.60); `runModelInference` v1.0.12 handles it unchanged and the app reads `predictions[0]`. The tab now shows a progress run-log, a cURL/Python/SQL payload panel, Databricks endpoint+model deep links, and a cleaned header. Remaining (user): publish `dist`, re-instantiate the App Studio card, hard-refresh, click-test live inference (should now show a nuanced %, not 0%).
4. **Lakebase Ops rebind — app config DONE, live test pending user.** `listScenarios`/`listPredictionFeedback`/`deleteScenario`/`savePredictionFeedback` are all mapped in `manifest.json`/`dist` (route to released 1.0.12). The `listScenarios` 400 is a stale-context issue; re-instantiating the App Studio card after publishing the current `dist` picks up the full `packageMapping` and clears it (live rows verified server-side: 4 scenarios, 6 feedback).
