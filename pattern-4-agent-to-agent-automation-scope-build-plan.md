...
- `pattern4ce` v1.0.5 (Lakebase read/write functions) is created but **not released**. Release requires explicit user approval per Code Engine safety rule; until then, Lakebase Ops uses preview fallback in the app.
...
4. DONE locally — Lakebase Ops: `p4_scenario_runs` + `p4_prediction_feedback` are created/seeded in `cobra-v1`; app CRUD UI and local `pattern4ce` functions are built; `pattern4ce` v1.0.5 is created and awaits explicit release approval for live reads/writes.
5. Validate the published Domo app after user publishes latest `dist` and after `pattern4ce` v1.0.5 is released: live Lakebase rows should load, scenario CRUD should write to `cobra-v1`, ML prediction feedback should save, and Genie result charts should remain visible.
